...
class Recorder extends StatefulWidget {
  final Function save;
  final int? maxLengthInSeconds;

  const Recorder({Key? key, required this.save, this.maxLengthInSeconds})
      : super(key: key);
  @override
...
// You can request multiple permissions at once.
  ...
  Widget build(BuildContext context) {
    if (_current != null) {
      Duration? _myDuration = _current?.duration;
      print(_myDuration);
      if ((_myDuration!) >= Duration(seconds: widget.maxLengthInSeconds!)) {
    ...
    return WillPopScope(
      onWillPop: () async => !(_currentStatus == RecordingStatus.Recording),
      child: Stack(
  ...
  _stop() async {
    ...
    setState(() {
      _current = result!;
      _currentStatus = _current!.status!;
      _current!.duration = Duration(milliseconds: 0);
      _recordIcon = Icons.mic;
      stop = false;
    });
  }
